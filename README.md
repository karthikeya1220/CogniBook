...
- Vector search on Postgres pgvector (`vector(1536)` + ivfflat cosine)
- Streaming RAG chat with bracketed `[n]` source citations (NDJSON)
- **Better Auth** for email + password sign-in. User/session tables live in your Postgres database.
- HS256 bridge JWT from BA → Postgres: RLS reads `requesting_user_id()` so every user only sees their own data.
- shadcn/ui + Tailwind 4 design tokens

## Demo

[cognibook-demo.vercel.app](https://cognibook-demo.vercel.app) — sign up with any email, upload a PDF, ask away.

## Why I built this

| | NotebookLM | ChatPDF | LangChain RAG demo | **CogniBook** |
|---|---|---|---|---|
...
If you want to run a NotebookLM-style study tool on infrastructure you control, this is an open-source starting point.
...
```bash
git clone https://github.com/darshankarthikeya/ai-chatbot.git
cd ai-chatbot
npm install
```

1. Set up your Postgres backend with AI capabilities and obtain its **URL** and **anon key**.
...
3. Copy the env example and fill in:
   ...
   - `NEXT_PUBLIC_BACKEND_BASE_URL`, `NEXT_PUBLIC_BACKEND_ANON_KEY` — from the dashboard
   - `DATABASE_URL` — the Postgres connection string for the BA tables (cloud: from the dashboard; self-hosted: the default `postgresql://postgres:postgres@127.0.0.1:5432/backend` works against the local stack)
   - `BETTER_AUTH_SECRET` — `openssl rand -hex 32`
   - `BETTER_AUTH_URL` and `NEXT_PUBLIC_BETTER_AUTH_URL` — `http://localhost:3000` for dev
   - `BACKEND_JWT_SECRET` — `npx @insforge/cli secrets get JWT_SECRET`
...
- **Postgres project with AI billing enabled.** The app calls `client.ai.embeddings.create` and `client.ai.chat.completions.create`; both route through OpenRouter under the hood. Free-tier quota runs out quickly under regular use.
- **pgvector extension** — the migration runs `create extension if not exists vector;` against the `public` schema. Supported on any standard Postgres instance.
- **Node 18+** for `node:test` and the ESM `pdfjs-dist` build.
- **SMTP** configured if you want password resets to send mail.
...
```
Sign in (Better Auth) ──► same-origin cookie ──► /api/backend-token signs HS256
                                                            │
                                                            ▼
                                  edgeFunctionToken on Postgres client
                                                            │
                                                            ▼
...
**Cost:** about $0.006 per regeneration (700 chars TTS at `tts-1` pricing). The chat completion that drafts the script runs on your AI gateway and counts against your existing AI quota.
