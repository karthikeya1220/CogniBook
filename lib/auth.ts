...
// the Backend data API by construction — no REVOKE step needed.
//
...
// rejects Backend cloud's self-signed Postgres cert with
...
export const auth = betterAuth({
  ...
  emailAndPassword: {
    enabled: true,
    // Wire BA's password-reset email through Backend's email service.
    // Requires SMTP configured on the Backend side (cloud: automatic;
    // self-hosted: PUT /api/auth/smtp-config).
    sendResetPassword: async ({ user, url }) => {
      const { serverMailer } = await import('./backend-server-mailer');
