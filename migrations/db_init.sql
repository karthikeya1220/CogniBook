...
-- 16MB default maintenance_work_mem on Backend cloud. Bump it for this
...
-- Backend data API automatically — no REVOKE step needed.
create schema if not exists better_auth;

-- RLS policies read the BA-signed `sub` claim via auth.jwt(). The HS256
-- bridge JWT minted by /api/backend-token (and lib/auth-state.ts) carries
...
-- storage.objects RLS — Backend fresh projects only ship a project_admin
